...
import {
  ...
  Tag, User, Calendar as CalendarIcon, Clock, HardDrive, Video, Loader2,
} from 'lucide-react';
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      ...
      if (!error && data) {
        setVideos(
          data.map((row: any) => {
            const sizeRaw = Number(row.size_mb ?? row.file_size_mb ?? row.file_size ?? 0);
            return {
              id: row.id,
              title: row.title ?? row.name ?? 'Untitled Video',
    ...
    >
      ...
      <div className={`grid grid-cols-3 gap-3`}>
        {[
          ...
          { label: 'Total Views', value: videos.reduce((a, b) => a + Number(b.views || 0), 0).toLocaleString() },
          { label: 'Storage Used', value: `${(videos.reduce((a, b) => a + Number(b.sizeRaw || 0), 0)).toFixed(1)} GB` },
        ].map((s, i) => (
          <motion.div
            key={i}
      ...
      {videos.length === 0 ? (
        <div className="rounded-2xl border border-dashed p-8 text-center">
          <EmptyState
            icon={Video}
            title="🎬 Video library is empty"
            description="Upload your first video to showcase your work."
          />
        </div>
      ) : (
