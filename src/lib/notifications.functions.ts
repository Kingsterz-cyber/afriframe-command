...
export const getNotificationDiagnostics = createServerFn({ method: "POST" })
  .inputValidator((data: { accessToken: string }) => {
    if (!data?.accessToken) throw new Error("Not signed in");
    return data;
  })
  .handler(async ({ data }) => {
    const { studioAdmin, notificationConfigStatus } = await import("./notifications.server");
    const { listSubscriptions, pushConfigStatus, pushDispatchUrl } = await import("./push.server");
    const db = studioAdmin();
    const { data: authData, error } = await db.auth.getUser(data.accessToken);
    if (error || !authData.user) return { ok: false as const, reason: "unauthorized" as const };

    const notificationStatus = notificationConfigStatus();
    const vapidStatus = pushConfigStatus();
    const devices = await listSubscriptions("admin");
    return {
      ok: true as const,
      ...
      status: {
        supabase: notificationStatus.supabase && notificationStatus.serviceRole,
        resend: notificationStatus.resend && Boolean(process.env["AFRIFRAME_EMAIL_FROM"]),
        vapid: vapidStatus.configured,
        dispatchUrl: Boolean(pushDispatchUrl()),
      },
    };
  });

export const testNotificationEmail = createServerFn({ method: "POST" })
  .inputValidator((data: { accessToken: string }) => {
    if (!data?.accessToken) throw new Error("Not signed in");
    return data;
  })
  .handler(async ({ data }) => {
    const { studioAdmin, sendEmail, shell } = await import("./notifications.server");
    const db = studioAdmin();
    const { data: authData, error } = await db.auth.getUser(data.accessToken);
    ...
    try {
      await sendEmail(
        authData.user.email,
        "Afriframe diagnostics email test",
        shell(
          "Email delivery is working",
          "<p>This real test confirms that Afriframe can deliver email from the new deployment.</p>",
        ),
      );
      return { sent: true as const, recipient: authData.user.email };
    } catch (err) {
      return { sent: false as const, reason: "send_failed" as const, error: String(err) };
    }
  });

export const testNotificationPush = createServerFn({ method: "POST" })
  .inputValidator((data: { accessToken: string }) => {
    if (!data?.accessToken) throw new Error("Not signed in");
    return data;
  })
  .handler(async ({ data }) => {
    ...
    if (error || !authData.user)
      return { sent: 0, failed: 0, devices: 0, reason: "unauthorized" as const };
    const devices = await listSubscriptions("admin");
    ...
    const result = await sendPush({
      title: "Afriframe diagnostics",
      body: "Web Push delivery is working.",
      url: "/notifications",
      tag: "afriframe-diagnostics",
    });
